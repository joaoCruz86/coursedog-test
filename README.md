...
The CI pipeline runs automatically on every push and pull request. Otherwise you can run it manually as per below steps:
...
### If run manually on github

1. Go to the **Actions** tab in this repository.
2. Select the **CI** workflow on top left.
...
│   ├── shop-cart.spec.ts          # 6 tests — add, remove, totals, duplicate alert
...
**26 tests total** across 6 spec files: 22 UI (chromium project) + 4 API (api project).
...
- Empty email field prevents login.
...
- Cart reflects correct products, prices and total for multiple items.
...
- *(fixme)* Empty cart should not allow proceeding to checkout: site bug, kept as `fixme` to show it.
...
- **Confirmation contains the correct price and shipping address** 
...
- Valid CSV passes schema validation and contains the expected first row. 
...
This split is what makes `npm test` work with **zero Docker setup**, you can clone, install, and run the main deliverable in under a minute. The API suite is opt-in via `npm run test:api` only when Docker is running.

### CI Configuration
[`playwright.config.ts`] is tuned for CI stability and clear diagnostics:

Workers: Reduced to 2 on CI to match runner capacity and avoid network-related flakiness.
Retries: Enabled (2) on CI to handle transient failures; disabled locally for fast feedback.
forbidOnly: Prevents accidental commits of test.only(...).
Artifacts: Traces and videos captured on first retry; screenshots on failure.
Reporting: list for CI logs, html for downloadable reports.
